...
int main()
{
    ...
    while (true)
    {
      ...
      int word_length = get_integer("Word Length: ");
        while (word_length <= 0 || word_length > 29 || word_length == 27 || word_length == 26)
        {
          cout << "Invalid Length" << endl;
          word_length = get_integer("Word Length: ");
        }
        ...
        bool word_options = get_yesno("Do you want to show the number of remaining words?");
        ...
        while (!game.is_won() && !game.is_lost())
        {
            ...
            //PRINT WORDS REMAINING IF SELECTED
            if (word_options)
            {
              cout << "There are " << game.get_words_remaining() << " words still possible." << endl << endl;
            }
            //Get the next guessed letter
            ...
            bool good_guess = game.process_guess_part2(guess);
            //game.print_remaining();
            if (good_guess)
            {
