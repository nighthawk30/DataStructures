...
void markov::run_one(string infile, int order, string model_type, int size,
    int seed)
{
  if (!_set_text(infile))
  {
...
void markov::set_order()
{
  ...
  {
    cout << "Invalid order.  Order is unchanged, still " << _order << "."
         << endl;
  }
  else
  {
...
void markov::set_random_seed()
{
  ...
  cout << "With a value of -1, the random sequence will be different each time."
       << endl;
  cout << "With any other integer value, the random sequence will be the same "
          "each time."
       << endl;
